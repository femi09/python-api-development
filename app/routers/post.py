...
from ..schema import PostCreate, PostResponse, UserCreate, UserResponse, PostVoteResponse
...
@router.get('/', status_code=status.HTTP_200_OK, response_model=List[PostVoteResponse])
...
@router.get('/my_posts', status_code=status.HTTP_200_OK, response_model=List[PostResponse])
...
@router.get('/{id}', status_code=status.HTTP_200_OK, response_model=PostVoteResponse)
...
async def delete_post(id: int,  db:Session = Depends(get_db), current_user = Depends(oauth2.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="no authorized to perform this action")
...
async def delete_post(id: int, new_post:PostCreate,  db:Session = Depends(get_db), current_user = Depends(oauth2.get_current_user)):
    post_query = db.query(models.Post).filter(models.Post.id == id)
    post = post_query.first()
